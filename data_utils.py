...
def load_data(tokenizer, file_path, batch_size, device):
    data = []
    ...
    while True:
        ...
        word_label = data_file.readline()[:-1]
        # print(text)
        # print(word)
        # print(word_label)
        if not text:
            break
        assert word_label in ['0', '-1', '1']
        labels = []
        for word in text.split(' '):
            if word == '$T$':
                tokenized_word = tokenizer.encode(target_word)
                labels += [label_dict[word_label]] * len(tokenized_word)
            else:
