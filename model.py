...
class BertAndLinear(nn.Module):
    def __init__(self, bert_model_type=None, bert_model_path=None, type_nums=7):
        ...
        self.fc = nn.Linear(self.bert_config.hidden_size, type_nums)  # 直接分类
...
class BertAndTransformer(nn.Module):
    def __init__(self, bert_model_type=None, bert_model_path=None, type_nums=7):
        ...
        if bert_model_path is not None:
            ...
            self.bert_config = BertConfig.from_pretrained(bert_model_type)
            self.bert = BertModel.from_pretrained(bert_model_type)
        self.tfm = nn.TransformerEncoderLayer(self.bert_config.hidden_size, nhead=8)  # 直接分类
...
class RoBERTaAndLinear(nn.Module):
    def __init__(self, roberta_model_type=None, roberta_model_path=None, type_nums=7):
        ...
        self.fc = nn.Linear(self.roberta_config.hidden_size, type_nums)  # 直接分类
